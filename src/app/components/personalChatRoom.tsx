...
import { Send, Smile, History, X, Minus } from "lucide-react";
...
export const FloatingDmPanel = ({ targetUser, onClose }: FloatingDmPanelProps) => {
  ...
  useEffect(() => {
    if (encryptedRoomId && encryptedUserId) {
      try {
        ...
        if (isNaN(decryptedRoomId) || isNaN(decryptedUserId)) throw new Error("Decryption resulted in invalid numbers");
  ...
  const handleSend = async () => {
    ...
    const { error: sendError } = await supabaseBrowserClient
      .from("personal_messages")
      .insert([{ room_id: roomId, from_id: userId, to_id: targetUser.id, message: text }]);
    if (sendError) console.error("Error sending message:", sendError.message);
    setOptimisticIds((prev) => { const next = new Set(prev); next.delete(optId); return next; });
  };
  ...
  if (minimized) {
    return (
      <button
        ...
        className="h-12 w-12 rounded-full bg-gradient-to-br from-purple-600 to-blue-600 shadow-lg shadow-purple-500/30 flex items-center justify-center text-white text-sm font-semibold hover:scale-105 transition-all animate-fade-in"
      >
        <Avatar className="h-10 w-10 border-2 border-white/30">
          <AvatarImage src={targetUser?.profile_pic} alt={targetUser?.user_name} className="object-cover" />
          <AvatarFallback className="text-[9px] bg-transparent text-white">
            {targetUser?.user_name?.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) || "?"}
          </AvatarFallback>
        </Avatar>
      </button>
    );
    ...
    <div className="w-[320px] sm:w-[360px] rounded-2xl border border-border/50 bg-card/95 backdrop-blur-xl shadow-2xl shadow-black/30 animate-fade-in overflow-hidden flex flex-col">
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-border/30 bg-card">
        ...
        <div className="flex items-center gap-0.5">
          <button onClick={() => setMinimized(true)} className="h-7 w-7 rounded-lg hover:bg-secondary/50 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
            <Minus className="h-3.5 w-3.5" />
          </button>
          <button onClick={onClose} className="h-7 w-7 rounded-lg hover:bg-secondary/50 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
              ...
              <div key={message.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                <div className={`px-3 py-1.5 rounded-2xl text-sm max-w-[85%] break-words ${
                  ...
                  <p className={`text-[10px] mt-0.5 ${isMine ? "text-white/50" : "text-muted-foreground"}`}>
                    {formatTime(message.sent_at)}{isOpt && " · sending..."}
                  </p>
                </div>
              </div>
            );
...
// Keep the old export for backwards compat but it's unused now
export const PersonalChatRoomPage = () => null;
